...
Logger.prototype = {
    ...
    doDump: function(level) {
        //alert(this.lastErrorMsg+"\n");
        if (this.level >= level) {
            var msg = this.name+": "+this.lastErrorMsg + "\n";
    ...
    debug: function(str) {
        ...
        this.doDump(logging.DEBUG);
    },
    info: function(str) {
        this.lastErrorMsg = str;
        this.doDump(logging.INFO);
    },
    warn: function(str) {
        this.lastErrorMsg = str;
        this.doDump(logging.WARN);
    },
    error: function(str) {
    ...
    exception: function(e, str) {
        ...
        if (typeof(str) == 'undefined') {
            str = logging.getObjectTree(e, true);
        } else {
            str = str +"\n"+ logging.getObjectTree(e, true);
        }
        this.lastErrorMsg = str;
        this.doDump(logging.EXCEPTION);
...
var logging = {
    ...
    getObjectTree: function(o, recurse, compress, level)
    {
        var s = "";
        var pfx = "";
                ...
                switch (t)
                {
                    ...
                    case "object":
                        ...
                        if (!compress)
                            s += pfx + "|\n";
                        if ((i != "parent") && (recurse))
                            s += logging.getObjectTree (o[i], recurse - 1,
                                                 compress, level + 1);
                        break;
